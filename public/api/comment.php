...
try {
    ...
    $stmt = $pdo->prepare("
        SELECT id, patient_id, doctor_id, visit_reason, appointment_date, start_time
        FROM appointments 
    ...
    // Insert comment
    $stmt = $pdo->prepare("
        INSERT INTO appointment_comments (appointment_id, user_id, message, created_at)
        VALUES (?, ?, ?, NOW())
    ");
    $stmt->execute([$appointmentId, $doctorId, $commentText]);
    $commentId = (int)$pdo->lastInsertId();

    // Fetch new comment
    $stmt = $pdo->prepare("
        SELECT id, message as comment_text, created_at
