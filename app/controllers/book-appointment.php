...
try {
    ...
    $stmt = $pdo->prepare("
        SELECT slot_time, is_break
        FROM time_slots
        WHERE doctor_id = ? AND slot_date = ? AND slot_time BETWEEN ? AND ?
    ");
    ...
    // Insert appointment
    $stmt = $pdo->prepare("
        INSERT INTO appointments (patient_id, doctor_id, appointment_date, appointment_time, status, visit_reason, duration_minutes)
        VALUES (?, ?, ?, ?, 'Pending', ?, ?)
    ");
    ...
    // Get doctor info
    $stmt = $pdo->prepare("SELECT name, fee FROM doctors WHERE id = ?");
