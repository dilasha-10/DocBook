...
$patient_name = htmlspecialchars($appt['patient_name'] ?? 'Patient');
$appt_date    = htmlspecialchars($appt['date'] ?? $appt['appointment_date'] ?? '');
$appt_start   = htmlspecialchars($appt['time'] ?? $appt['start_time'] ?? '');
...
.msg-row.doctor  .msg-content { align-items: flex-end; }
.msg-row.patient .msg-content { align-items: flex-start; }
.msg-bubble {
...
.msg-time { font-size: 11px; color: var(--hint); padding: 0 4px; }
.chat-empty-state {
    flex: 1; display: flex; flex-direction: column;
    align-items: center; justify-content: center;
    color: var(--muted); gap: 10px; padding: 40px 0;
}
...
.chat-textarea {
    flex: 1; padding: 11px 16px; font-size: 15px; font-family: inherit;
...
ob_start();
$patient_initials = strtoupper(substr(strip_tags($patient_name), 0, 2));
$doctor_initials  = strtoupper(substr($doctor['name'] ?? 'DR', 0, 2));
?>
...
<div class="chat-page-wrap">
    <div class="chat-main">
        ...
        <div class="chat-header">
            <a href="<?= BASE_URL ?>/doctor/schedule" class="chat-back-btn"><i class="fa fa-arrow-left"></i> Back</a>
            <div class="chat-doctor-avatar"><?= $patient_initials ?></div>
...
$js_patient_name     = json_encode($patient_name);
$js_patient_initials = json_encode($patient_initials);
$js_doctor_initials  = json_encode($doctor_initials);

$extra_scripts = <<<JS
<script>
const APPT_ID       = {$appt_id};
const DOCTOR_ID     = {$doctor_id};
const PATIENT_INIT  = {$js_patient_initials};
const DOCTOR_INIT   = {$js_doctor_initials};
...
function buildBubble(msg) {
    ...
    const side     = isDoctor ? 'doctor' : 'patient';
    const avClass  = isDoctor ? 'doctor-av' : 'patient-av';
    const initials = isDoctor ? DOCTOR_INIT : PATIENT_INIT;
...
function loadMessages() {
    fetch(BASE_URL + '/doctor/api/messages/' + APPT_ID)
        .then(r => r.json())
        .then(d => {
            if (!d.success) return;
